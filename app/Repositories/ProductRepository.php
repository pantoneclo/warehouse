...
class ProductRepository extends BaseRepository
{
    ...
    public function storeProduct($input)
    {
        try {
            ...
            $product_abstract_id = $input['product_abstract_id'] ?? null;
            $code = Product::generateProductCodeByValue($product_abstract_id,$variant_id);
            ...
            $productAdd =[
                ...
                'notes' => $input['notes'],
                'product_abstract_id' => $input['product_abstract_id'],
                // 'variant_id' => $input['variant_id']?? $variant->id,
                'variant_id' => $input['variant_id'],
            ];
            // dd ($productAdd);
           $productInputArray = Arr::only($productAdd, [
                'name', 'code', 'date','product_cost','product_price','stock_alert',
                'notes','product_abstract_id','variant_id'
            ]);
            // dd ($productInputArray);
            $product = $this->create( $productInputArray);
            // dd($product);
