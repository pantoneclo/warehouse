...
export const warehouseProductsSearch = (params, warehouseId) => async (dispatch) => {
    dispatch(setLoading(true));

    try {
        const response = await apiConfig.get(apiBaseURL.WAREHOUSE_PRODUCTS_SEARCH, { params, warehouseId });
        
        console.log("Response from API:", response); // Add this line to log the response

        if (response && response.data.data) {
            dispatch({ type: advancedSearchActionType.FETCH_WAREHOUSE_PRODUCTS_SEARCH, payload: response.data.data });
