    ...
    return (
        <Dropdown className='table-dropdown'>
            ...
            <Dropdown.Menu>
                ...
                {
                goToEditProduct && isEditMode !== false&& !item.is_sale_created && item.is_return !== 1 &&
                // item.name === 'admin' || isEditMode === false&&goToEditProduct && !item.is_sale_created && item.is_return !== 1  ? null :
                <Dropdown.Item onClick={(e) => {
                    e.stopPropagation();
                ...
                { item.name === 'super_admin' || item.email === 'superadmin@bitnix.ai' || isDeleteMode === false ? null :
                <Dropdown.Item onClick={(e) => {
                    e.stopPropagation();
