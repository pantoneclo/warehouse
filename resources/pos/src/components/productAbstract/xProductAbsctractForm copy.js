...
useEffect (() => {
    if (singleProduct && singleProduct[0]) {
        console.log(sizeOptions, 'sizeOptions2')
        handleAttributeChange( defaultOption,sizeOptions,colorOptions );  
    }
 ...
 // value={[
    ...
    const handleAttributeChange = (newAttributes) => {
        ...
        const variantsElements = newAttributes.map((item, index) => (
            ...
            console.log(colorOptions, 'color option3'),
            console.log(sizeOptions, 'size option3'),
            ...
            <div className='col-12 mb-3' key={index}>
                <label
                    className='form-label' htmlFor={`select-${index}`}>{_.startCase(item.value)}</label>
                    {
                        ...
                        singleProduct && singleProduct[0] && singleProduct[0] ?
                        <CreatableSelect
                        id={`select-${index}`}
                        ...
                        value={singleProduct[0].attributes[item.value].map((item) => { return { value: item, label: item } }) }
    ...
    const handleSelectChange = (index, newValue) => {
        console.log({ newValue ,index },'new value and index' );
        if (index === 0) {
