...
const User = (props) => {
    ...
    const delete_permission = usePermission(Permissions.USER_DELETE);
    const create_permission = usePermission(Permissions.USER_CREATE);
    ...
    const columns = [
        ...
        },
        {
            ...
            allowOverflow: true,
            button: true,
            cell: row => <ActionButton item={row} goToEditProduct={goToEdit} isEditMode={row.edit_permission}
    ...
    return (
        <MasterLayout>
            ...
            <ReactDataTable columns={columns} items={itemsValue} onChange={onChange}
                            ButtonValue={create_permission?getFormattedMessage('user.create.title'):null}
