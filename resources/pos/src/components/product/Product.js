...
const Product = (props) => {
    ...
    const goToProductDetailPage = (ProductId) => {
        window.location.href = '#/app/products/details/' + ProductId;
    };
    const goToVariantAddScreen = (item) => {
        const id = item.id;
        window.location.href = '#/app/products/variants/' + id;
        ...
        return (
            {
                ...
                variant_size: product?.attributes.variant?.size,
                variant_color: product?.attributes.variant?.color,
            }
        )
    });
    ...
    const columns = [
        ...
        },
        {
            ...
            cell: row => {
                return (
                    <span style={{fontSize:'14px',width:'200px', maxWidth:'200px', whiteSpace: 'normal' , overflow:'hidden'}} >
        ...
        {
            name: 'Style',//getFormattedMessage('product.input.style.label'),
        ...
        {
            name: 'Variant',//getFormattedMessage('globally.react-table.column.created-date.label'),
            selector: row => row.variant_size,
            sortField: 'created_at',
            sortable: false,
            cell: row => {
                return (
                    <span style={{fontSize:'10px',width:'100px', maxWidth:'100px', whiteSpace: 'normal' , overflow:'hidden'}} className='badge bg-light-info'>
                        <div className='mb-1'>Size: {row.variant_size}</div>
        ...
        },
        {
            ...
            goToVariantAddScreen={goToVariantAddScreen}
            isVariantAddMode={false}
            isVariantAddIcon={false}
                 />
        }
    ];
