...
import { fetchAllProductAbstracts } from './../../store/action/productAbstractAction';
...
const ProductForm = (props) => {
    ...
    const [selectedVariant,setVariant] = useState(singleProduct && singleProduct[0] ? (singleProduct[0].variant_id) : null);
    const [selectedAbstract,setAbstract] = useState(singleProduct && singleProduct[0] ? (singleProduct[0].product_abstract_id) : null);

    const [selectedTax] = useState(newTax && newTax[0] ? ([{ label: newTax[0].label, value: newTax[0].value }]) : null);
    // const [selectedVariant] = useState(newTax && newTax[0] ? ([{label: newTax[0].label, value: newTax[0].value}]) : null);
    ...
    useEffect(() => {
        if (singleProduct) {
            ...
            setProductValue({
                ...
                product_abstract_id: singleProduct ? singleProduct[0].product_abstract_id : '',
                variant_id: singleProduct ? singleProduct[0].variant_id : '',
    ...
    const options = productAbstracts.length && productAbstracts?.map((product) => ({
        label: product.attributes.name+ ' ('+product?.attributes.pan_style+') ',
        value: product.id,
    }));

    const options2 = variants.length && variants?.map((variant) => {
        const variantValues = Object.entries(variant.attributes.variant).map(([key, value]) => `${key}: ${value}`).join(', ');
        return {
            label: variantValues ?? '', // Use an empty string as the default value
            value: variant.id,
        };
    });
    ...
    const handleProductSelect = (selectedOption) => {
        setProductValue({ ...productValue, product_abstract_id: selectedOption });
        setAbstract(selectedOption)

    };
    const handleVariantSelect = (selectedOption2) => {
        setProductValue({ ...productValue, variant_id: selectedOption2 });
        setVariant(selectedOption2)
    ...
    const handleValidation = () => {
        ...
        // }
        else if (!productValue['product_abstract_id']) {

            errorss['product_abstract_id'] = getFormattedMessage('product.input.product-category.validate.label');
        }
    ...
    const prepareFormData = (data) => {
        console.log(data.product_abstract_id.value, 'data')
        const formData = new FormData();
        formData.append('name', data.name);
        // formData.append('code', data.code);
        formData.append('product_abstract_id', data.product_abstract_id.value);
        formData.append('variant_id', data.variant_id.value);
                        ...
                        <div className='col-xl-8'>
                            <div className='card'>
                                <div className='row'>
                                    ...
                                    <div className='col-md-6 mb-3'>
                                        <label
                                            className='form-label'>{getFormattedMessage('product.input.abstract.label')}: </label>
                                        <span className='required' />
                                        <Select
                                            options={options}
                                    ...
                                    <div className='col-md-6 mb-3'>
                                        <label
                                            className='form-label'>{getFormattedMessage('product.input.variant.label')}: </label>
                                        <span className='required' />
                                        <Select
                                            options={options2}
...
const mapStateToProps = (state) => {
    ...
    const { brands, variants, productCategories, productAbstracts, units, totalRecord, suppliers, warehouses, productUnits, frontSetting } = state;
    return { brands, variants, productCategories, productAbstracts, units, totalRecord, suppliers, warehouses, productUnits, frontSetting };
};
