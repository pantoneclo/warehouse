...
const SalesForm = (props) => {
    ...
    const prepareFormData = (prepareData) => {
        const formValue = {
            ...
            payment_status: prepareData.payment_status?.value ?? prepareData.payment_status ?? 2,
            payment_type: prepareData.payment_type?.value ?? prepareData.payment_type ?? 0,
            parcel_company_id: prepareData.parcel_company_id?.value ?? prepareData.parcel_company_id ?? '',
            parcel_number: prepareData.parcel_number,
            shipment_id: prepareData.shipment_id ? prepareData.shipment_id : '',
            country: prepareData.country?.value ?? prepareData.country ?? '',
            order_no: prepareData.order_no,
            market_place: prepareData.market_place?.label ?? prepareData.market_place ?? '',
